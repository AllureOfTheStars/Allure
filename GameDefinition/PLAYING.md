...
The focus is on resource management and survival, including food gathering
...
the relevant lore details, including crafting recipes afforded
by this terrain, if any. The basic terrain kinds are as follows.
...
The current pointman's melee bonus (`xdy + %`, which means rolling 
...
you want to stash something in the shared inventory from the pointman's
outfit).
...
the ground, press `M`, and point in the direction of that tile. 
The components will be consumed, and the resulting item will appear. 
...
(including the rare stimpacks crystals) you manage to plunder and carry off.
Note that engaging in wanton slaughter is counterproductive in such a setting,
because it wastes time and non-unique enemies never carry valuables,
but stash them instead.
...
When all your heroes fall, you lose, but the heroes are going
to be eventually found and brought back to life using the extremely
expensive nanomedicine perfected for space travel hibernation.
...
for yourself new immediate goals, look for resources to attain them.
...
or use mouse. Feel free to recall the lore pertinent to a position
and use the lore menu by pressing `~`.
